...
export default function EntradasClient({ transactions, categories }: { transactions: Transaction[], categories: Category[] }) {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-0.5">Gerencie suas receitas e ganhos</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="card p-4">
          ...
          <p className="text-xl font-bold text-green-600 mt-1">{formatCurrency(total)}</p>
        </div>
        <div className="card p-4">
      ...
      <TransactionList transactions={filtered} categories={categories} type="entrada" />

      {showModal && (
        <Modal title="Nova entrada" onClose={() => setShowModal(false)}>
          <TransactionForm type="entrada" categories={categories} onSuccess={() => setShowModal(false)} />
        </Modal>
      )}
    </div>
  )
